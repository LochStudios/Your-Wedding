...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($username === null) {
            $stmt = $conn->prepare('INSERT INTO clients (display_name, password_hash) VALUES (?, ?)');
            $stmt->bind_param('ss', $displayName, $hash);
        } else {
            $stmt = $conn->prepare('INSERT INTO clients (username, display_name, password_hash) VALUES (?, ?, ?)');
            $stmt->bind_param('sss', $username, $displayName, $hash);
        }
        if (!$stmt->execute()) {
...
$clients = [];
$stmt = $conn->prepare('SELECT id, username, display_name, created_at FROM clients ORDER BY created_at DESC');
$stmt->execute();
            ...
            <div class="container">
                ...
                <form method="post">
                    <div class="field">
                        ...
                        <div class="control">
                            <input class="input" type="text" name="display_name" placeholder="John &amp; Jane" required />
                        </div>
                    </div>
                    <div class="field">
                        <label class="label">Username (optional)</label>
                        <div class="control">
                            <input class="input" type="text" name="username" placeholder="john@example.com" />
                        </div>
